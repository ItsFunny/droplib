...
var _ services.IStateStore = (*dbStore)(nil)

// NewStore creates the dbStore of the state pkg.
func NewDefaultStateStore(db services.DB) services.IStateStore {
	return dbStore{db}
}

// LoadStateFromDBOrGenesisFile loads the most recent state from the database,
// or creates a new one from the given genesisFilePath.
func (store dbStore) LoadFromDBOrGenesisFile(genesisFilePath string) (models.LatestState, error) {
	state, err := store.Load()
	if err != nil {
		return models.LatestState{}, err
	}
	if state.IsEmpty() {
		var err error
		state, err = models.MakeGenesisStateFromFile(genesisFilePath)
		if err != nil {
			return state, err
		}
	}
...
func (store dbStore) LoadFromDBOrGenesisDoc(genesisDoc *models.GenesisDoc) (models.LatestState, error) {
	state, err := store.Load()
	if err != nil {
		return models.LatestState{}, err
	}
	...
	if state.IsEmpty() {
		state = models.MakeGenesisState(genesisDoc)
	}
...
func (store dbStore) Load() (models.LatestState, error) {
	return store.loadState(stateKey)
}

func (store dbStore) loadState(key []byte) (state models.LatestState, err error) {
	...
	sp := new(states.LatestStateProto)

	err = commonutils.UnMarshal(buf, sp)
	if err != nil {
		// DATA HAS BEEN CORRUPTED OR THE SPEC HAS CHANGED
		fmt.Sprintf(`LoadState: Data has been corrupted or its spec has changed:
		%v\n`, err)
		os.Exit(-1)
	}

	sm, err := models.LatestStateFromProto(sp)
	if err != nil {
		return state, err
	}
...
func (store dbStore) Save(state models.LatestState) error {
	return store.save(state, stateKey)
}

func (store dbStore) save(state models.LatestState, key []byte) error {
	...
	if nextHeight == 1 {
		nextHeight = int64(state.InitialHeight)
		// This extra logic due to Tendermint validator set changes being delayed 1 block.
		// It may get overwritten due to InitChain validator updates.
		if err := store.saveValidatorsInfo(int64(nextHeight), int64(nextHeight), state.Validators, batch); err != nil {
			return err
		}
	}
	// Save next validators.
	err := store.saveValidatorsInfo(int64(nextHeight+1), state.LastHeightValidatorsChanged, state.NextValidators, batch)
	if err != nil {
		return err
	}

	// Save next consensus params.
	if err := store.saveConsensusParamsInfo(int64(nextHeight),
...
func (store dbStore) Bootstrap(state models.LatestState) error {
	...
	if height > 1 && !state.LastValidators.IsNilOrEmpty() {
		if err := store.saveValidatorsInfo(int64(height-1), int64(height-1), state.LastValidators, batch); err != nil {
			return err
		}
	}

	if err := store.saveValidatorsInfo(int64(height), int64(height), state.Validators, batch); err != nil {
		return err
	}

	if err := store.saveValidatorsInfo(int64(height+1), int64(height+1), state.NextValidators, batch); err != nil {
		return err
	}

	if err := store.saveConsensusParamsInfo(int64(height),
		state.LastHeightConsensusParamsChanged, state.ConsensusParams, batch); err != nil {
		return err
	}
...
func (store dbStore) PruneStates(retainHeight uint64) error {
	...
	// return an error
	if err := store.pruneConsensusParams(int64(retainHeight)); err != nil {
		return err
	}

	if err := store.pruneValidatorSets(int64(retainHeight)); err != nil {
		return err
	}

	if err := store.pruneABCIResponses(int64(retainHeight)); err != nil {
		return err
	}
...
func (store dbStore) pruneConsensusParams(retainHeight int64) error {
	...
	// is dependent on.
	if paramsInfo.ConsensusParams.Equal(&types.ConsensusParamsProto{}) {
		...
		if err != nil || lastRecordedConsensusParams.ConsensusParams.Equal(&types.ConsensusParamsProto{}) {
			return fmt.Errorf(
...
func (store dbStore) reverseBatchDelete(batch services.Batch, start, end []byte) ([]byte, error) {
...
func (store dbStore) LoadValidators(height int64) (*models.ValidatorSet, error) {
	...
	if valInfo.ValidatorSet == nil {
		...
		vs, err := models.ValidatorSetFromProto(valInfo2.ValidatorSet)
		if err != nil {
			return nil, err
		}

		vs.IncrementProposerPriority(libs.SafeConvertInt32(height - lastStoredHeight)) // mutate
		vi2, err := vs.ToProto()
		if err != nil {
			return nil, err
		}
	...
	vip, err := models.ValidatorSetFromProto(valInfo.ValidatorSet)
	if err != nil {
		return nil, err
	}
...
func lastStoredHeightFor(height, lastHeightChanged int64) int64 {
	...
	return libs.MaxInt64(checkpointHeight, lastHeightChanged)
}

// CONTRACT: Returned ValidatorsInfo can be mutated.
func loadValidatorsInfo(db services.DB, height int64) (*states.ValidatorsInfoProto, error) {
	...
	v := new(states.ValidatorsInfoProto)
	err = commonutils.UnMarshal(buf, v)
	if err != nil {
		// DATA HAS BEEN CORRUPTED OR THE SPEC HAS CHANGED
		libs.Exit(fmt.Sprintf(`LoadValidators: Data has been corrupted or its spec has changed:
                %v\n`, err))
	}
	// TODO: ensure that buf is completely read.
...
func (store dbStore) saveValidatorsInfo(
	...
	bz, err := commonutils.Marshal(valInfo)
	if err != nil {
		return err
	}
...
var (
	empty   = models.ConsensusParams{}
	emptypb = types.ConsensusParamsProto{}
)

// LoadConsensusParams loads the ConsensusParams for a given height.
func (store dbStore) LoadConsensusParams(height int64) (models.ConsensusParams, error) {
	...
	return models.ConsensusParamsFromProto(paramsInfo.ConsensusParams), nil
}

func (store dbStore) loadConsensusParamsInfo(height int64) (*states.ConsensusParamsInfoProto, error) {
	...
	paramsInfo := new(states.ConsensusParamsInfoProto)
	if err = commonutils.UnMarshal(buf, paramsInfo); err != nil {
		// DATA HAS BEEN CORRUPTED OR THE SPEC HAS CHANGED
		fmt.Println(fmt.Sprintf(`LoadConsensusParams: Data has been corrupted or its spec has changed:
                %v\n`, err))
		os.Exit(-1)
...
func (store dbStore) saveConsensusParamsInfo(
	...
	params models.ConsensusParams,
	batch services.Batch,
) error {
	paramsInfo := &states.ConsensusParamsInfoProto{
	...
	bz, err := commonutils.Marshal(paramsInfo)
	if err != nil {
		return err
	}
