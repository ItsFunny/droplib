...
# -*- coding: utf-8 -*-
# @Author : joker
# @Time : 2021/4/7 6:08 上午
# @File : block_db_impl.go
# @Description :
# @Attention :
*/
package block
...
var _ services.DB = (*GoLevelDB)(nil)
...
func (db *GoLevelDB) NewBatch() services.Batch {
	return newGoLevelDBBatch(db)
}

// Iterator implements DB.
func (db *GoLevelDB) Iterator(start, end []byte) (services.Iterator, error) {
...
func (db *GoLevelDB) ReverseIterator(start, end []byte) (services.Iterator, error) {
	if (start != nil && len(start) == 0) || (end != nil && len(end) == 0) {
		return nil, errKeyEmpty
	}
	itr := db.db.NewIterator(&util.Range{Start: start, Limit: end}, nil)
