...
// NOTE: it expects the LeafHash and the elements of Aunts to be of size libs.Size,
...
func (sp *Proof) ValidateBasic() error {
	...
	if len(sp.LeafHash) != libs.Size {
		return fmt.Errorf("expected LeafHash size to be %d, got %d", libs.Size, len(sp.LeafHash))
	}
	...
	for i, auntHash := range sp.Aunts {
		if len(auntHash) != libs.Size {
			return fmt.Errorf("expected Aunts#%d size to be %d, got %d", i, libs.Size, len(auntHash))
		}
	}
	return nil
}

func (sp *Proof) ToProto() *crypto.ProofProto {
	if sp == nil {
		return nil
	}
	pb := new(crypto.ProofProto)
...
func ProofFromProto(pb *crypto.ProofProto) (*Proof, error) {
	if pb == nil {
