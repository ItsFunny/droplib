...
func ValueOpDecoder(pop crypto.ProofOpProto) (ProofOperator, error) {
	if pop.Type != ProofOpValue {
		return nil, fmt.Errorf("unexpected ProofOp.Type; got %v, want %v", pop.Type, ProofOpValue)
	}
	var pbop crypto.ValueOpProto // a bit strange as we'll discard this, but it works.
...
func (op ValueOp) ProofOp() crypto.ProofOpProto {
	pbval := crypto.ValueOpProto{
		Key:   op.key,
		Proof: op.Proof.ToProto(),
	}
	// bz, err := pbval.Marshal()
	bz, err := protobuflibs.Marshal(&pbval)
	if err != nil {
		panic(err)
	}
	return crypto.ProofOpProto{
		Type: ProofOpValue,
		Key:  op.key,
		Data: bz,
	}
