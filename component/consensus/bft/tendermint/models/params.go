...
func (params ConsensusParams) HashConsensusParams() []byte {
	hasher := libs.New()
	hp :=types.HashedParamsProto{
		BlockMaxBytes: params.Block.MaxBytes,
		BlockMaxGas:   params.Block.MaxGas,
	}

	// bz, err := hp.Marshal()
	bz, err := commonutils.Marshal(&hp)
	if err != nil {
		panic(err)
	}
...
func (params *ConsensusParams) Equals(params2 *ConsensusParams) bool {
	return params.Block == params2.Block &&
		...
		libs.StringSliceEqual(params.Validator.PubKeyTypes, params2.Validator.PubKeyTypes)
}

// Update returns a copy of the params with updates from the non-zero fields of p2.
// NOTE: note: must not modify the original
func (params ConsensusParams) UpdateConsensusParams(params2 *types.ConsensusParamsProto) ConsensusParams {
	...
	if params2.Evidence != nil {
		...
		res.Evidence.MaxAgeDuration = commonutils.ConvtDuration2TimeDuration(params2.Evidence.MaxAgeDuration)
...
func (params *ConsensusParams) ToProto() *types.ConsensusParamsProto {
	return &types.ConsensusParamsProto{
		Block: &types.BlockParamsProto{
			MaxBytes: params.Block.MaxBytes,
			MaxGas:   params.Block.MaxGas,
		},
		Evidence: &types.EvidenceParamsProto{
			MaxAgeNumBlocks: params.Evidence.MaxAgeNumBlocks,
			MaxAgeDuration:  commonutils.ConvTimeDuration2Duration(params.Evidence.MaxAgeDuration),
			MaxBytes:        params.Evidence.MaxBytes,
		},
		Validator: &types.ValidatorParamsProto{
			PubKeyTypes: params.Validator.PubKeyTypes,
		},
		Version: &types.VersionParamsProto{
			AppVersion: params.Version.AppVersion,
		},
	}
}

func ConsensusParamsFromProto(pbParams *types.ConsensusParamsProto) ConsensusParams {
	return ConsensusParams{
		Block: BlockParams{
		...
		Evidence: EvidenceParams{
			...
			MaxAgeDuration:  commonutils.ConvtDuration2TimeDuration(pbParams.Evidence.MaxAgeDuration),
