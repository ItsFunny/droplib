...
func (q *QueryImpl) Conditions() ([]Condition, error) {
	var (
		eventAttr string
		op        Operator
	)
...
func (q *QueryImpl) Matches(events map[string][]string) (bool, error) {
	if len(events) == 0 {
		return false, nil
	}
